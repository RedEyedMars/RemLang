...
public abstract class as_context extends class_body_context{
	...
	public void parse_as_statement() {
		...
		if(brace_7.containsKey(_position)) {
			...
			if(_state==SUCCESS&&brace_7.get(_position_as_statement_brace)==_position) {
				_position+=1;
			}
			else {
				_state=SUCCESS;
				_result_acceptor.add(_result);
				_result_acceptor.add(new Parser.Result.Fail.EOB("as_statement",_position,_lineNumberRanges));
				_position=brace_7.get(_position_as_statement_brace)+1;
				_succeedOnEnd=false;
			}
