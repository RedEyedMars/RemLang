...
public class number_context extends base_context{
	...
	public void parse_NUMBER(){
		...
		int _state_9 = _state;
		if(_position<_inputLength){
			int _position_of_last_success_10 = _position;
			if(_position<_inputLength){
			...
			if(_multiple_index_10==0 ){
				_state=FAILED;
			}
			if(_state==FAILED){
				_position=_position_of_last_success_10;
			}
		}
		if(_state_9==SUCCESS){
			_state=SUCCESS;
