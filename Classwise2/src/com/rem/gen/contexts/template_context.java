...
public abstract class template_context extends comments_context{
		...
		if(brace_4.containsKey(_position)) {
			...
			if(_pass==SECOND_PASS) {
				...
				if(_state==SUCCESS&&brace_4.get(_position_template_parameters_brace)==_position) {
					...
					_result_acceptor.add(new Parser.Result.Fail.EOB("template_parameters",_position,_lineNumberRanges));
